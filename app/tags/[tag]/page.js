...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `#${tag} | A Conversation About AI`,
    description: `Articles related to the tag "${tag}".`,
    openGraph: {
      title: `#${tag} | A Conversation About AI`,
      description: `Articles related to the tag "${tag}".`
    }
...
export default async function TagPage({ params, searchParams }) {
  ...
  const posts = await getTagData(tag);
    ...
    <div className="space-y-12">
      ...
      <section className="pb-6 border-b border-white/10">
        ...
        <p className="mt-3 text-gray-300 max-w-2xl">
          Articles tagged with <span className="text-sky-300">#{tag}</span> in the world of AI.
        </p>
      </section>
      ...
      <section>
        {paginated.length === 0 ? (
          <p className="text-gray-400">No articles found for this tag.</p>
        ) : (
